...
{
    public:
        ThreadData(int nrarity, int nuniques, int niterations){
            rarity = nrarity;
            uniques = nuniques;
...
int main(int argc, char* argv[]){
    ...
    if(argc >=5){
        ...
        std::cout << "Usage " << argv[0] << " <number of simulations> <number of threads> <number of uniques> <rarity of uniques>" << std::endl;
        return -1;
    }
    if(argc == 6){
        fileName = argv[5];
    }
    std::printf("Running %d Simulations of %d slots with 1/%d rarity on %d threads.\n", sims, uniques, rarity, threads);
    ...
    for(int i = 0; i < threads; i++)
        threadArguments.push_back(new ThreadData(rarity, uniques, (sims/threads)+1));
...
void* runIteration(void* data){
    ...
    std::uniform_int_distribution<int> chanceDistrib(1,rarity);
